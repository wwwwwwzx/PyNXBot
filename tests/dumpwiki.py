		...
		for ii in range(tablelength):
			...
			if entry1.Probabilities(star) > 0:
				...
				if entry1.Species() == entry2.Species() and entry1.AltForm() == entry2.AltForm() and entry1.IsGigantamax() == entry2.IsGigantamax() and entry1.Field12() == entry2.Field12():
					...
					msg += getform_short(entry1.Species(),entry1.IsGigantamax(),entry1.AltForm(),entry1.Field12() == 2) + "}}"
					print(msg)
				else:
					msg = header + getspecies_short(entry1.Species(),entry1.IsGigantamax(),entry1.AltForm())
					...
					msg += getform_short(entry2.Species(),entry2.IsGigantamax(),entry2.AltForm(),entry2.Field12() == 2) + "}}"
					print(msg)
	print('{{捕捉/footer|红}}')
else: # Full version
	print('__TOC__')
